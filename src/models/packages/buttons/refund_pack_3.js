...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const cur = row.reffund_pack_3;
      if (typeof cur === 'number') row.reffund_pack_3 = cur + 1;
      else row.reffund_pack_3 = 1;

      try {
        await fs.writeFile(dbPath, JSON.stringify(data, null, 2), 'utf8');
      } catch (e) {
        console.error('[refund_pack_3] failed to write DB:', e);
      }

      const newContent = 'You have selected "refund pack 3"';
      try {
      ...
      console.error('[refund_pack_3] error:', err);
