...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const cur = row.reffund_pack_4;
      if (typeof cur === 'number') row.reffund_pack_4 = cur + 1;
      else row.reffund_pack_4 = 1;

      try {
        await fs.writeFile(dbPath, JSON.stringify(data, null, 2), 'utf8');
      } catch (e) {
        console.error('[refund_pack_4] failed to write DB:', e);
      }

      const newContent = 'You have selected "refund pack 4"';
      try {
      ...
      console.error('[refund_pack_4] error:', err);
