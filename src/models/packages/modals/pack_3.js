    ...
    async execute(interaction) {
        try {
            ...
            try {
                ...
                if (idx !== -1) {
                    const current = channels[idx].pack_3;
                    channels[idx].pack_3 = (typeof current === 'number') ? current + 1 : 1;
                } else {
                    channels.push({ channel_id: chanId ? String(chanId) : null, channel_name: interaction.channel?.name || null, user_id: interaction.user?.id || null, user_name: interaction.user?.username || null, pack_3: 1 });
                }
                fs.writeFileSync(channelDbPath, JSON.stringify(channels, null, 2));
            } catch (e) { console.error('Failed to persist pack_3 counter:', e); }
                ...
                if (maybeMessageId && interaction.channel) {
                    ...
                    try {
                        if (fs.existsSync(packsPath)) {
                            ...
                            const packEntry = (packs && packs.find && packs.find(x => x && (x.pack_3 || x['pack_3']))) || null;
                            const maybe = packEntry ? (packEntry.pack_3 || packEntry['pack_3']) : null;
                            if (maybe) { packXp = Number(maybe.xp) || 0; packName = maybe.name || packName; }
                        }
                    } catch (pe) { console.error('Failed reading packs.json for pack_3:', pe); }
            ...
            } catch (e) { console.error('Failed to fetch/edit original message for pack_3 modal:', e); }

            try { if (!interaction.replied && !interaction.deferred) { await interaction.deferReply({ ephemeral: true }); setTimeout(async () => { try { await interaction.deleteReply(); } catch {} }, 250); } } catch (e) { console.error('Failed to defer/delete ephemeral reply for modal_pack_3:', e); }
        } catch (err) { console.error('Error handling modal_pack_3 submit:', err); try { if (!interaction.replied && !interaction.deferred) await interaction.reply({ content: 'An error occurred while processing your submission.', ephemeral: true }); } catch (e) { console.error('Failed to send error reply for modal_pack_3:', e); } }
    },
};
